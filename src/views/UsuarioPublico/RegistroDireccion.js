...
export const RegistroDireccion = forwardRef((props, ref) => {
  ...
  const { getMunicipioEstado, municipiosListId } = useContext(
    MunicipiosContext
  );
  const { localidadesList, getLocalidadesMunicipio } = useContext(
    LocalidadesContext
  );
  const { getCoberturaProgramas, coberturalist } = useContext(
    RegistroSolicitudContext
  );
  ...
  useEffect(() => {
    ...
    console.log(
      "ESTO LLEGO DE LA CONSULTA DE LA DIRECCION DEL BENEFICIARIO -----> ",
      direccionBeneficiario
    );
    if (direccionBeneficiario !== undefined) {
      console.log(
        "DIRECCION DEL BENEFICIARIO -----> ",
        direccionBeneficiario[0]
      );
      if (direccionBeneficiario[0] !== undefined) {
        ...
        getLocalidadesMunicipio(
          direccionBeneficiario[0].idMunicipio,
          direccionBeneficiario[0].codigoPostal
        );
  ...
  const llenado = () => {
    if (beneficiario != undefined) {
      ...
      if (beneficiario[0] !== undefined) {
        console.log(
          "LLEGA EL ID DEL BENEFICIARIO AL LLENAR LOS DATOS? ---> ",
          beneficiario[0].id
        );
