...
export const DialogAgregarArchivos = (props, { etapaSeleccionada }) => {
  ...
  const { documentosApoyoList, getDocumentosApoyoByTipoReq } = useContext(
    RegistroCargaDocumentosContext
  );
  ...
  const urlEndpointNumHojas =
    baseUrl + `/documentosExpediente/obtenerUltimaHoja/`;
  ...
  useEffect(() => {
    if (props.etapaSeleccionada != undefined) {
      ...
      let idTipoRequerimiento =
        props.etapaSeleccionada.idEtapa ===
        "05fcf1ef-b99d-46a2-974f-f04f5419d5ac"
          ? "c946c03b-eae1-4ee1-aa93-d99c08825f97"
          : "36bd3924-24aa-4ce6-bbad-2c4bdbf5ed82";
      GetNumHojas();
      GetSubEtapas();
  ...
  const handleInputNombre = (event) => {
    setDocumentoSeleccionado(
      documentosApoyoList.filter((e) => e.id === event.target.value)[0].idDocumentoRequisito
    );
  ...
  const handleChangeFileAdd = (event) => {
    ...
    if (event.target.files[0].type == tipoDocument.extension) {
      ...
      errors.selectedFile =
        "El tipo de archivo requerido es " + tipoDocument.extension;
      console.log(
        "archivo invalido",
        event.target.files[0].type,
        tipoDocument.extension
      );
  ...
  const handleAgregar = (e) => {
    ...
    console.log("TIPO DE DOCUMENTO =>", tipoDocument);
    ...
    dataModel.dsobservaciones = documentoSeleccionado;
      ...
      >
        ...
        <form onSubmit={handleAgregar}>
          <DialogTitle id="form-dialog-title">
            Registrar Información de expediente{" "}
          </DialogTitle>
          <DialogContent>
            <TextField
              disabled
          ...
          <DialogContent>
            <TextField
              ...
              defaultValue={moment(initExpDig.fechaCreacion).format(
          ...
          <DialogContent>
            ...
            >
              ...
              {etapasHija.map((item) => (
                <MenuItem key={item.idEtapa} value={item.idEtapa}>
                  {item.dsetapa}
                </MenuItem>
              ))}
            </TextField>

            {errors.subClasificacion && (
              <FormHelperText error={errors.subClasificacion}>
                {errors.subClasificacion}
              </FormHelperText>
            )}
          </DialogContent>
          ...
          <DialogContent>
            ...
            {errors.nombreDoc && (
              <FormHelperText error={errors.nombreDoc}>
                {errors.nombreDoc}
              </FormHelperText>
            )}
          </DialogContent>
          <DialogContent>
            ...
            {errors.fechaDocumento && (
              <FormHelperText error={errors.fechaDocumento}>
                {errors.fechaDocumento}
              </FormHelperText>
            )}
          </DialogContent>
          <DialogContent>
            ...
            {errors.observaciones && (
              <FormHelperText error={errors.observaciones}>
                {errors.observaciones}
              </FormHelperText>
            )}
          </DialogContent>
          <DialogContent>
            ...
            />
            {errors.selectedFile && (
              <FormHelperText error={errors.selectedFile}>
                {errors.selectedFile}
              </FormHelperText>
            )}
          </DialogContent>
