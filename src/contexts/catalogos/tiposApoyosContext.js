...
export const TiposApoyosContextProvider = (props) => {
  ...
  const getTiposApoyosActivos = async () => {
    try {
      const { page, size } = state;
      const result = await axiosGet(`tiposApoyos/search/findByActivoTrue`);
      console.log(result._embedded);
      console.log(result._embedded.tiposApoyos);
      dispatch({
        type: GET_TIPOS_APOYOS_ACTIVOS,
        payload: result,
      });
    } catch (error) {
      console.log(error);
  ...
  const getTiposApoyosByParametros = async (search) => {
    try {
      const result = await axiosGet(
        `tiposApoyos/search/findByDstipoapoyoContaining?dstipoapoyo=${search}`
      );
